...
"dayzHumanity"		addPublicVariableEventHandler {(_this select 1) call player_humanityChange};
...
if (isServer) then {
	...
	"dayzDeleteObj"		addPublicVariableEventHandler {_id = (_this select 1) call local_deleteObj};
	...
	"dayzUpdate"		addPublicVariableEventHandler {_id = (_this select 1) call dayz_processUpdate};
	"dayzLoginRecord"	addPublicVariableEventHandler {_id = (_this select 1) call dayz_recordLogin};
...
if (!isDedicated) then {
	...
	"dayzFire"			addPublicVariableEventHandler {nul=(_this select 1) spawn BIS_Effects_Burn}; // BIS Func has errors so, call will spam-lock the client
