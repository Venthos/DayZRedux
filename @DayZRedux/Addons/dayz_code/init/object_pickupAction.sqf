...
actionMonitor = {
	...
	while {(_run) and (canPickup)} do {
		if ((alive _holder) and (canPickup)) then {
			_distance = player distance _holder;
			// Add action to player
			if ((_distance < 1.75) and (_action == -1) and (canPickup)) then {
        ...
        sleep 2;
			...
			if ((_distance > 100) and (canPickup)) then {
...
if ((_classname == "WoodenArrow") and (canPickup)) then {
